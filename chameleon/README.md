...
**Batch stain normalisation for H&E and IHC brightfield histology images**
...
- **Four normalisation modes** spanning two algorithms (histogram specification and Reinhard color transfer) with two reference strategies (explicit reference image or batch-derived synthetic reference)
...
- **CSV normalisation log** with per-image, per-channel statistics (mean, std, Wasserstein distance or delta-E)
...
| **4** | Reinhard color transfer | Batch-average synthetic reference | Unbiased Reinhard normalisation; no reference needed |
...
![Inspector screenshot coming soon](docs/inspector_screenshot.png)

<!-- Remove the line above and uncomment below once you have a screenshot:
![Pre-flight Inspector](docs/inspector_screenshot.png)
-->
...
- A **2×2 grid** of all four normalised results — large enough to see staining differences clearly
...
1. Browse to your **input folder** — supported images load automatically
2. For Modes 1 or 3: browse to a **reference image**
3. Browse to an **output folder**
4. Click **🔍 Preview All Methods** to compare all four normalisations side by side
5. Select your preferred method and click **✔ Apply & Close**
6. Click **▶ Run Normalisation**
...
Normalised images are saved to the chosen output folder with `_norm` appended to the filename.

If **Save CSV normalisation log** is enabled, a timestamped log is written alongside the images.
...
The **Wasserstein distance** quantifies how much the distribution was shifted — higher values indicate images that were strong outliers before normalisation. The **delta-E** is the mean per-pixel LAB colour difference between original and normalised.
...
@software{turner2026chameleon,
  author  = {Turner, Neill},
  title   = {Chameleon: Batch Stain Normalisation for Histology Images},
  year    = {2026},
...
- Macenko stain separation as a fifth normalisation mode
