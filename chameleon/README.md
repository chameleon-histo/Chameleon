...
```
output = (source − src\_mean) / src\_std × tgt\_std + tgt\_mean
```
...
!\[Pre-flight Inspector](docs/preview\_screen.png)
...
4. Click **🔍 Preview All Methods** to compare all four normalizations side by side
5. Select your preferred method and click **✔ Apply \& Close**
6. Click **▶ Run normalization**
...
│   └── run\_Chameleon.m         ← launcher
└── Python/
    ├── normalizer\_app.py       ← GUI application
    ├── normalizer\_core.py      ← algorithms (no GUI dependency)
    ├── run\_normalizer.py       ← launcher / PyInstaller entry point
    └── requirements.txt
```
